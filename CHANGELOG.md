...
### Fixed
- **Mobile chat horizontal overflow eliminated.** Chat message bubbles (user, assistant, streaming) now clip content with `overflow: hidden`, preventing `<hr>` separators and wide inline elements from bleeding past the bubble boundary. `overflow-wrap: anywhere` on `.markdown-content` breaks long box-drawing character sequences (e.g. `────────`) that previously pushed the page layout wider than the viewport. `MobileShell` gains `contain: layout` so the off-screen detail panel (`translateX(100%)`) no longer inflates `scrollWidth` to double the viewport. `overflow-x: clip` on `html`/`body` (stronger than `hidden` for CSS transform overflow) prevents page-level horizontal scroll on iOS Safari. All 10 sessions in the regression scan show `docScrollWidth === viewportWidth` after these fixes.
- **Auth bypass via tunnel fixed; zrok v2 API supported.** The loopback-IP bypass in `auth-plugin.ts` (which skips auth on requests from `127.0.0.1`) could be exploited through a zrok tunnel — zrok proxies all traffic from localhost, so remote tunnel users passed the IP check even with OAuth configured. The bypass now also checks the `Host` header and refuses loopback-skip when `Host` is a non-localhost domain. zrok v2 API compatibility: `env.kind === 'v2'` tunnels no longer call the deprecated `reserve` command, `--force-local` flag added, and the share URL regex updated for the `shares.zrok.io` domain format.
